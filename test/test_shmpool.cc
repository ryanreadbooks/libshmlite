...
TEST(ShmPoolTest, FuncTest_Get_type_not_match) {
  ...
  ASSERT_EQ(*int_d2, *reinterpret_cast<int *>(&d2_value)); // 将double型的内存位置强行用int去解析,会造成截断，仅保留了低位

  long *long_d2 = GET_LONG("d2"); // d2 = 856.36
  std::cout << std::hex << std::ios::showbase << "long_d2 = " << *long_d2 << '\n';
  ASSERT_EQ(*long_d2, *reinterpret_cast<long *>(&d2_value)); // 将double型的内存位置强行用long去解析
